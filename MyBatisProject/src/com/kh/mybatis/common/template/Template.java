...
public class Template {
	 ...
	 * 2. close()함수 : 전달받은 JDBC객체를 반납시키는 구문
	...
	public static SqlSession getSqlSession() {
		...
		// SqlSessionFactory객체를 생성하기 위해서는 SqlSessionFactoryBuilder객체가 필요
...
//			SqlSessionFactory ssf = ssfb.build(stream);
			// 3단계 SqlSession객체 생성 및 트랜잭션 처리를 자동으로할지 수종으로할지 매개변수를 통해 구분(false:수동/true:자동)
//			sqlSession = ssf.openSession(false); // 자동커밋을 막아주는 변수(false)
